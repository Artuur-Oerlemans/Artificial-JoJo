...
var token = "NTE4MzQ0Mjg3NTU0MTA5NDUw.DuQYpw.Ibtms2TwuW1agDtb2-tQ-HL9Jl4";
var client = new discord.Client();
client.memory = require("./memory.json");

var Currency = require("./currency.js");

var shoutOra = require("./shoutOra.js");

client.login(token);
client.on("ready", function () {
  console.log("ready");
  client.user.setActivity("Oh, That's A Baseball!", {
    type: "PLAYING"
  });
}); //the thing that should be infront of commands

var prefix = ";";
client.on('error', console.error); // what to do when a message is received

client.on("message", function (message) {
  // make certain the bot doesn't fall into a loop with another bot.
  if (message.author.bot) return; // check if a command was used
...
function executeCommands(message) {
  ...
  var cmd = args[0]; // now the first args will be the thing following the command
...
function tellLoavesEaten(channel, yearsUnfiltered) {
  ...
  var menacing = client.emojis.find(function (emoji) {
    return emoji.name === "menacing";
  });
  var years = parseInt(yearsUnfiltered);
...
function showCountdownNextEpisode(channel) {
  ...
  channel.send(differenceDatesDHHMMSS(now, nextEpisodeDate) + " until the next episode of JoJo's bizarre adventure part 5: Golden Wind.");
...
function getNextDate(day, hour) {
  var now = new Date();
  var nextTime = new Date(); //go to correct day of the week

  nextTime.setDate(nextTime.getDate() + (day + 7 - nextTime.getDay()) % 7); //if that is after the hour, go to next week.
...
function differenceDatesDHHMMSS(date1, date2) {
  var output = "";
  var differenceMilliseconds = date2.getTime() - date1.getTime();
  var oneSecond = 1000;
  var oneMinute = 60 * oneSecond;
  var oneHour = 60 * oneMinute;
  var oneDay = 24 * oneHour;
  var seconds = Math.floor(differenceMilliseconds / oneSecond % 1000);
  var minutes = Math.floor(differenceMilliseconds / oneMinute % 60);
  var hours = Math.floor(differenceMilliseconds / oneHour % 24);
  var days = Math.floor(differenceMilliseconds / oneDay);
...
function showProgress(channel) {
  var progress = client.memory["inventory"].lires;
  var leaderBoard = getLeaderBoard("lires");
  ...
  embed.addField("Passione top 3", "**1.** " + leaderBoard[0].displayName + " " + leaderBoard[0].goods.toLocaleString() + " lire\n2. " + leaderBoard[1].displayName + " " + leaderBoard[1].goods.toLocaleString() + " lire\n2. " + leaderBoard[2].displayName + " " + leaderBoard[2].goods.toLocaleString()) + " lire";
  channel.send(embed);
} // check for donations


function executeDonations(message) {
  var currency = Currency.detectCurrency(message.content);
  if (currency == null) return;
  if (currency.valueInLires > 0) message.channel.send(thankYouMessage(message.author));
...
function receiveDonation(contributor, channel, change) {
  var inBank = client.memory["inventory"].lires;
  var afterTransfer = inBank;
...
function thankYouMessage(user) {
  var displayName = user.lastMessage.member.displayName;
  var responses = ["GURETO DESU-YO!", "Naissu", "Grazie", "Berry naissu " + displayName + "-chan", "Hey baby", "OH MY GOD!!!", "MUDA JANAI", "Kore wa ii", "*yEy!* Fine, Thank You!", "Arigotou gozaimasu", "GOURUDO EKUSUPERIENSU", "At this rate we will be at 10,000,000,000 lire in now time"];
...
function updateInventory(contributor, goods, change) {
  ...
  var inBank = client.memory["inventory"][goods];
  var afterTransfer = inBank + change;
  client.memory["inventory"][goods] = afterTransfer; // register the contribution

  var displayName = contributor.lastMessage.member.displayName; // check if the user is already in the database

  if (!client.memory["contributors"][contributor.id]) client.memory["contributors"][contributor.id] = {
    displayName: displayName
  }; // check if previous contributions have already with these goods

  var afterContribution = client.memory["contributors"][contributor.id][goods] ? client.memory["contributors"][contributor.id][goods] + change : change;
  client.memory["contributors"][contributor.id][goods] = afterContribution; // sync to memory.json

  fs.writeFile("./memory.json", JSON.stringify(client.memory, null, 4), function (err) {
    if (err) throw err;
  });
...
function getRanking(id, goods) {
  ...
  for (var rank = 1; rank <= leaderBoard.length; rank++) {
    if (leaderBoard[rank - 1].id == id) return {
      rank: rank,
      goods: leaderBoard[rank - 1].goods
    };
  }
...
function getLeaderBoard(goods) {
  var contributors = client.memory["contributors"];
  var leaderBoard = Object.keys(contributors).map(function (key) {
    return {
      id: key,
      displayName: this[key].displayName,
      goods: this[key][goods] ? this[key][goods] : 0
    };
  }, contributors);
  leaderBoard.sort(function (p1, p2) {
    return p2.goods - p1.goods;
  });
  return leaderBoard;
}